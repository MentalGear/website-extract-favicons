#!/bin/bash
# Final fixed deterministic test suite for favicon extraction using local server
set -euo pipefail
...
EOF
    ...
    # Test Case 6: Invalid Manifest with proper JSON
...
EOF
    ...
    # Create invalid JSON (malformed)
    cat > invalid-manifest-test/invalid-manifest.json << 'EOF'
{
  "name": "Test App",
  "icons": "this is invalid - should be array"
}
EOF
...
assert_url_contains() {
    ...
    full_output=$(timeout 30 bash "$SCRIPT_TO_TEST" -v "$url_to_test" 2>&1)
    exit_code=$?
...
assert_no_url_returned() {
    ...
    set +e
    output=$(timeout 30 bash "$SCRIPT_TO_TEST" "$url_to_test" 2>/dev/null || true)
    exit_code=$?
    set -e
...
# Main test runner
run_tests() {
    echo "Running Final Fixed Favicon Extraction Test Suite"
    echo "================================================="
