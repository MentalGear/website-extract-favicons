#!/bin/bash
# Corrected test suite for favicon extraction using local server
set -euo pipefail
...
# Function to extract final URL from script output
extract_final_url() {
    local full_output="$1"
    # The script outputs the URL as the last non-debug line
    # It could be after "[SUCCESS] Final icon URL:" or just a plain URL line
    echo "$full_output" | grep -v "^\[" | grep "^http" | tail -n1 || echo ""
}
...
EOF
    ...
    # Create invalid JSON (malformed structure)
...
# Test functions with corrected output parsing
assert_url_contains() {
    local url_to_test=$1
    ...
    # Extract the final URL from output using corrected logic
    output=$(extract_final_url "$full_output")
...
assert_apple_touch_icon_priority() {
    ...
    final_url=$(extract_final_url "$full_output")
...
# Main test runner
run_tests() {
    echo "Running Corrected Favicon Extraction Test Suite"
    echo "==============================================="
